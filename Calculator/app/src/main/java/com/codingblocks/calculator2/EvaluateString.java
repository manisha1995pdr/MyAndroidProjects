...
public class EvaluateString {
    public static float evaluate(String expression) {
        ...
        for (int i = 0; i < tokens.length; i++) {
            ...
            // Current token is an operator.
                ...
                while (!ops.empty() && hasPrecedence(tokens[i], ops.peek())) {
                    ...
                    values.push(applyOp(ops.pop(),first, second));
                }
        ...
        //In case of the expression like (58+,57-,58*)
        float first;
        float second;
        char operator;
        while (!ops.empty()) {
            first = values.pop();
            operator = ops.pop();
            if ((operator == '*' || operator == '/') && values.size() == 0) {
                second = 1;
            ...
            values.push(applyOp(operator, first, second));
        }
        return values.pop();
    }
